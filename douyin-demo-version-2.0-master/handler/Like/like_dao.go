...
func (d LikeDAO) CancelLike(videoid int64, userid int64) error {
	var like Like
	return dal.DB.Where(&Like{VideoID: string(videoid), UserID: string(userid)}).Delete(&like).Error
}

func (d LikeDAO) QueryLikeList(userid int64) ([]Video.Video, error) {
	var videoid []int64
	var videoList []Video.Video
	err := dal.DB.Where("user_id = ?", userid).Find(&videoid).Error
