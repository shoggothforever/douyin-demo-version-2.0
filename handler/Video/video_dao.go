...
func (v *VideoDao) QueryVideoListLogin(userid int64, last_time time.Time) (*VideoList, error) {
	...
	videolist.Videos = make([]*Video, 0, dal.MaxVideoList)
	err := dal.DB.Where("publish_time<? AND user_id=?", last_time, userid).Order("publish_time desc").Limit(dal.MaxVideoList).Find(&videolist.Videos).Error
	if err != nil {
		return nil, err
	}
...
func (v *VideoDao) QueryVideoListUnLogin(last_time time.Time) (*VideoList, error) {
	var videolist VideoList
	videolist.Videos = make([]*Video, 0, dal.MaxVideoList)
	err := dal.DB.Where("publish_time<?", last_time).Order("publish_time desc").Limit(dal.MaxVideoList).Find(&videolist.Videos).Error
	if err != nil {
		return nil, err
	}
	return &videolist, nil
...
func (v *VideoDao) PersistNewVideo(title string, userid int64, code int64, videoname, imagename string, user *user.UserInfoDao) error {
	...
	return dal.DB.Create(video).Error
}
...
func (v *VideoDao) GetUserVideoCode(userid int64) (int64, error) {
	var videocode int64
	err := dal.DB.Select("videocode").Where("userid=?", userid).First(&videocode).Error
...
func (v *VideoDao) QueryUserPublishList(userid int64) (*VideoList, error) {
	var videos []*Video
	err := dal.DB.Where("userid=?", userid).Find(&videos).Error
	if err != nil {
		return nil, err
	}
...
func QueryVideoById(vid int64) (Video, error) {
	var video Video
	err := dal.DB.Where("videoid = ?", vid).First(&video).Error
	return video, err
}
